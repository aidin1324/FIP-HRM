...
const FilterIndicator = React.memo(({ filter }) => {
  const filterLabel = filter === 'daily' ? 'По дням' : 'По месяцам';
  return (
    <span className="ml-2 text-xs font-normal bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 py-1 px-2 rounded-full">
      {filterLabel}
    </span>
  );
});
...
function DashboardCard01() {
  ...
  const { filter, setFilter, feedbackData, isLoading } = useContext(FilterContext);
  ...
  const processDataForChart = useMemo(() => {
    const processDailyData = (data) => {
      ...
      const values = validFeedbacks.map(feedback => feedback.waiter_score.score);
      
      return { labels, values };
    };

    const processMonthlyData = (data) => {
      const validFeedbacks = data.filter(feedback => feedback.waiter_score?.score !== null);
      ...
      const monthlyRatings = validFeedbacks.reduce((acc, feedback) => {
        ...
        if (!acc[monthKey]) {
          acc[monthKey] = {
            totalScore: 0,
            count: 0,
            month: date.toLocaleString('ru-RU', { month: 'long', year: 'numeric' }),
            date: date 
          };
        }
      ...
      const monthsData = Object.values(monthlyRatings)
        .sort((a, b) => a.date - b.date);
      
      const labels = monthsData.map(item => item.month);
      const values = monthsData.map(item => +(item.totalScore / item.count).toFixed(2));
  ...
  const updateChartData = useCallback((data, option) => {
  ...
  const handleFilterChange = useCallback((option) => {
    ...
    updateChartData(dataToUse, option);
  }, [feedbacks, feedbackData, setFilter, updateChartData]);
  ...
  const averageRating = useMemo(() => {
    ...
    const validFeedbacks = dataToUse.filter(feedback => 
      feedback.waiter_score?.score !== null
    );
    
    if (!validFeedbacks.length) return 'N/A';
    
    const sum = validFeedbacks.reduce((total, feedback) => 
      total + feedback.waiter_score.score, 0
    );
  ...
  const totalEntries = useMemo(() => {
    ...
    return dataToUse.filter(feedback => 
      feedback.waiter_score?.score !== null
    ).length;
  }, [feedbacks, feedbackData]);
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-sm rounded-xl p-4 relative">
      {(loading || isLoading) && <LoadingOverlay fullScreen={true} />}
      ...
      <header className="flex justify-between items-start mb-4">
        ...
        <div className="flex items-center space-x-2">
          <EditMenu align="right" className="relative inline-flex">
            <li>
              <Link
                className={`font-medium text-sm py-1 px-3 flex ${
                  filter === 'daily' 
      ...
      <div className="flex items-center space-x-4 mb-4">
        <div className="flex flex-col">
          ...
          <span className="text-2xl font-bold text-gray-800 dark:text-gray-100">
            {(loading || isLoading) ? "—" : averageRating}
          </span>
        </div>
        <div className="flex flex-col">
          <span className="text-sm font-semibold text-gray-400 dark:text-gray-500 uppercase">
          ...
          <span className="text-2xl font-bold text-gray-800 dark:text-gray-100">
            {(loading || isLoading) ? "—" : totalEntries}
          </span>
        </div>
      </div>
