...
describe('TelegramConfig', () => {
  beforeEach(() => {
    ...
    global.fetch.mockImplementation((url) => {
      if (url.includes('/config_json/config/telegram_chat_ids') && !url.includes('DELETE')) {
      ...
      } else if (url.includes('/config_json/config/telegram_chat_ids') && url.includes('DELETE')) {
        return Promise.resolve({
          ok: true,
          json: async () => true
        });
  ...
  test('должен показывать ошибку при вводе невалидного ID чата', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('ID чата должен быть числом')).toBeInTheDocument();
    });
  ...
  test('должен открывать модальное окно при нажатии на кнопку удаления', async () => {
    ...
    const deleteButtons = document.querySelectorAll('button[class*="text-red-500"]');
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Вы действительно хотите удалить ID чата/)).toBeInTheDocument();
